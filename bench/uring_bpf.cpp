...
long sys_set_bpf_level(int fd, int level) {
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < num_file; ++i) {
		...
		sys_set_bpf_level(s->fd_arr[i], level);
	}
	...
	while (s->finished_op < iteration * batch_size) {
		...
		for (int i = 0; i < reaped; ++i) {
			...
			if (batch_index == iteration - 1) {
				/* no further request */
				continue;
			}
			long next_batch_index = batch_index + 1;
